...
# 嘗試 import picamera2（推薦，新版 Pi OS）
try:
...
# 嘗試 import 舊版 picamera（PiCamera legacy stack）
try:
    import picamera
...
class LoadStreams:
    ...
    def __init__(self, sources="file.streams", img_size=640, stride=32, auto=True,
                 transforms=None, vid_stride=1,
                 raspi_resolution=(640, 480), raspi_fps=30):
        """
        Parameters
        ----------
        raspi_resolution : tuple
            Raspberry Pi Camera 的解析度 (width, height)。
        raspi_fps : int
            Raspberry Pi Camera 的目標 FPS。
        """
        ...
        # 記錄每個 source 使用的後端，供 update() 分支判斷
        self._raspi_cams = [None] * n   # 儲存 Picamera2 / PiCamera 物件

        for i, s in enumerate(sources):
            st = f"{i + 1}/{n}: {s}... "

            if is_raspi_source(s):
                # ── Raspberry Pi Camera 路徑 ──────────────────────────────
                use_v2 = s.lower() == "picamera2"

                if use_v2:
                    assert HAS_PICAMERA2, (
                        "請先安裝 picamera2：sudo apt install -y python3-picamera2"
                    )
                    cam = Picamera2()
                    config = cam.create_video_configuration(
                        main={"size": raspi_resolution, "format": "RGB888"},
                        controls={"FrameRate": raspi_fps}
                    )
                    cam.configure(config)
                    cam.start()
                    # 取得第一幀確認相機正常
                    frame = cam.capture_array()          # RGB, HWC
                    self.imgs[i] = frame[..., ::-1].copy()  # 轉 BGR 與 cv2 一致
                    w, h = raspi_resolution
                    self._raspi_cams[i] = ("picamera2", cam)

                else:
                    assert HAS_PICAMERA, (
                        "請先安裝 picamera：sudo pip install picamera"
                    )
                    cam_obj = picamera.PiCamera()
                    cam_obj.resolution = raspi_resolution
                    cam_obj.framerate = raspi_fps
                    time.sleep(2)  # 相機暖機
                    raw = picamera.array.PiRGBArray(cam_obj, size=raspi_resolution)
                    cam_obj.capture(raw, format="bgr", use_video_port=True)
                    self.imgs[i] = raw.array.copy()
                    w, h = raspi_resolution
                    self._raspi_cams[i] = ("picamera", cam_obj)

                self.frames[i] = float("inf")
                self.fps[i] = raspi_fps
                LOGGER.info(f"{st} Success (RPi Camera {w}x{h} at {raspi_fps} FPS)")

            else:
                # ── 原始通用路徑（YouTube / IP camera / 本地攝影機）────────
                ...
                self._raspi_cams[i] = ("cv2", cap)
                LOGGER.info(f"{st} Success ({self.frames[i]} frames {w}x{h} at {self.fps[i]:.2f} FPS)")

            # 啟動讀取執行緒
            self.threads[i] = Thread(
                target=self.update, args=(i, self._raspi_cams[i], s), daemon=True
            )
    ...
    # ------------------------------------------------------------------ #
    def update(self, i, cam_info, stream):
        """讀取影格並更新 self.imgs[i]，支援 picamera2 / picamera / cv2。"""
        backend, cam = cam_info
        n, f = 0, self.frames[i]
            ...
            while n < f:
                ...
                if n % self.vid_stride == 0:
                    try:
                        ...
                        self.imgs[i] = frame[..., ::-1].copy()  # → BGR
                    except Exception as e:
                        LOGGER.warning(f"WARNING ⚠️ picamera2 capture failed: {e}")
                        self.imgs[i] = np.zeros_like(self.imgs[i])
                time.sleep(0.0)

        elif backend == "picamera":
            import picamera.array
            raw = picamera.array.PiRGBArray(cam, size=cam.resolution)
            for _ in cam.capture_continuous(raw, format="bgr", use_video_port=True):
                n += 1
                if n % self.vid_stride == 0:
                    self.imgs[i] = raw.array.copy()
                raw.truncate(0)
...
# ---------------------------------------------------------
    ...
    def __next__(self):
        ...
        if not all(x.is_alive() for x in self.threads) or cv2.waitKey(1) == ord("q"):
            # 釋放 RPi Camera 資源
            for backend, cam in self._raspi_cams:
                if backend == "picamera2":
                    cam.stop()
                elif backend == "picamera":
                    cam.close()
